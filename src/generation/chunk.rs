...
pub fn generate_chunk(
    ...
    let biome = registries.biomes.get("caves").unwrap();
    let chunks_entity = chunks_query.single();

    let element = registries.materials.get("stone").unwrap();
    let mut chunks_to_add = HashMap::new();

    for ev in ev_chunkgen.read() {
        let part_position = ev.0;

        if chunk_manager.chunks.contains_key(&part_position)
            || chunks_to_add.contains_key(&part_position)
        {
            continue;
        }

        let Some(part) = tile_generator.get_tile(part_position, biome) else {
            continue;
        };
        ...
        for (x, y) in (-1..tile_generator.scale + 1).cartesian_product(-1..tile_generator.scale + 1)
        {
                ...
                || position.cmpeq(IVec2::ONE * tile_generator.scale).any()
            {
                if let Some(chunk) = chunk_manager
                    .get_chunk_mut(&(part_position + position))
                    .or(chunks_to_add.get_mut(&(part_position + position)))
                {
            ...
            (0..tile_generator.scale).cartesian_product(0..tile_generator.scale)
        {
            ...
            for (x, y) in (0..CHUNK_SIZE).cartesian_product(0..CHUNK_SIZE) {
                ...
                let in_part_position = ((chunk_position + part_position).rem_euclid(IVec2::ONE * tile_generator.scale)).as_vec2()
