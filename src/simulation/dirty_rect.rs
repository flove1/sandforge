...
pub fn dirty_rects_gizmos(mut gizmos: Gizmos, dirty_rects_resource: Res<DirtyRects>) {
    dirty_rects_resource.current.iter().for_each(|(position, rect)| {
        gizmos.rect_2d(
            position.as_vec2() + (rect.center().as_vec2() + Vec2::ONE / 4.0) / (CHUNK_SIZE as f32),
            0.0,
...
impl DirtyRects {
    ...
    pub fn request_update(&mut self, position: IVec2) {
        update_dirty_rects(
            &mut self.current,
            position.div_euclid(IVec2::splat(CHUNK_SIZE)),
            position.rem_euclid(IVec2::splat(CHUNK_SIZE)).as_uvec2()
    ...
    match chunk_offset {
        ...
        IVec2::ONE | IVec2::NEG_ONE => {
            for (x, y) in (-1..=1).cartesian_product(-1..=1) {
                let (chunk_position, cell_position) = modify_local_position(
                    chunk_position,
                    cell_position,
                    ivec2(x, y)
                );
